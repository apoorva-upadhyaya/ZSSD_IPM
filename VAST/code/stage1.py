...
class CreateDataset(Dataset):
    def __init__(self, texts,rationals,targets, comet_intents,llm_intents,stances,tokenizer, max_length):
        self.texts = texts
    ...
    def __getitem__(self, idx):
        ...
        rational_tokens = self.tokenizer.tokenize(rational)
...
data=pd.read_csv("../data/rational_clean_train.csv", delimiter=";") 
...
train_rationals=train_rationals[:50]
...
data=pd.read_csv("../data/rational_clean_test.csv", delimiter=";") 
...
train_dataset = CreateDataset(train_texts, train_rationals, train_targets, train_comet_intents, train_llm_intents,train_stances ,tokenizer, max_length=256)
val_dataset = CreateDataset(val_texts, val_rationals, val_targets, val_comet_intents, val_llm_intents,val_stances, tokenizer, max_length=256)
test_dataset = CreateDataset(test_texts, test_rationals, test_targets, test_comet_intents, test_llm_intents, test_stances, tokenizer, max_length=256)
...
dropout=0.5
# Initialize model
rational_model = RationalModel(bert_model, hidden_dim_rational,rank,num_heads,hidden_size,dropout,output_size)
...
loaded_rational_model = RationalModel(bert_model, hidden_dim_rational, rank,num_heads,hidden_size,dropout,output_size)
...
with torch.no_grad():
    for text_ids, text_mask, comet_intent_ids, comet_intent_mask, llm_intent_ids, llm_intent_mask, rational_binary,stance in test_loader:
        ...
        rational_logits = rational_model(text_ids, text_mask, comet_intent_ids, comet_intent_mask, llm_intent_ids, llm_intent_mask)
        text_ids = text_ids.squeeze(1)
        rational_output_tokens = backtrack_rational_output(text_ids, rational_logits, tokenizer)
