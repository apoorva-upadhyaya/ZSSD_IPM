...
class CreateDataset(Dataset):
    def __init__(self, ids, texts,llm1_rationals,llm2_rationals,aux_features,targets,comet_intents,llm_intents,true_stances,llm1_stances,llm2_stances,tweet_tokenizer,tokenizer, max_length):
        self.ids = ids
        self.texts = texts
    ...
    def __getitem__(self, idx):
        ...
        llm1_rational_tokens = self.tweet_tokenizer.tokenize(llm1_rational)
        ...
        llm2_rational_tokens = self.tweet_tokenizer.tokenize(llm2_rational)
...
class ConfidenceEnsemble(nn.Module):
    def __init__(self, num_models, confidence_threshold):
        super(ConfidenceEnsemble, self).__init__()
        self.num_models = num_models
        self.confidence_threshold = confidence_threshold
        
    def forward(self, model_outputs, confidence_scores):
        # Check confidence scores
        predictions=[]
        if torch.all(confidence_scores >= self.confidence_threshold):
            return model_outputs[0]
        else:
            for i in range(len(confidence_scores)):
                score=confidence_scores[i]
                if score>=self.confidence_threshold:
                    predictions.append(model_outputs[0][i])
                else:
                    input_=torch.tensor([model_outputs[0][i],model_outputs[1][i],model_outputs[2][i]])
                    output=self.majority_vote(input_)
                    predictions.append(output)

            # print("predictions::",predictions)
            return torch.tensor(predictions)

    def majority_vote(self, model_outputs):
        # Perform majority voting along the first dimension
        majority_output, _ = torch.mode(model_outputs, dim=0)
...
class StanceModel_Stage2(nn.Module):
    def __init__(self, bertweet_model,rank,num_heads,hidden_size,num_intermediate_outputs,dropout,num_classes):
        ...
        # Ensemble output
        self.ensemble_output=ConfidenceEnsemble(num_intermediate_outputs,0.7)
        # Final Linear layer
        self.fc1 = nn.Linear(4*hidden_size,rank)
        self.fc2 = nn.Linear(hidden_size, num_classes)

    def forward(self,text_ids,text_mask,aux_feature,loaded_rational_preds,llm1_rational_binary,llm2_rational_binary,llm1_stance,llm2_stance):

        text_ids = text_ids.squeeze(1)
        text_mask = text_mask.squeeze(1)
        text_output = self.bertweet(input_ids=text_ids, attention_mask=text_mask)[0][:, :, 0] # [CLS] token
        
        aux_features_modified = aux_feature.repeat(1, 128, 1)
        ...
        probabilities = torch.softmax(stance_output, dim=-1)
        probabilities,_ = torch.max(probabilities, dim=-1)
        # print("max probabilities::::",probabilities.tolist())
        predictions = torch.argmax(stance_output, dim=1)
        output=self.ensemble_output([predictions,llm1_stance,llm2_stance],probabilities)
...
for i in range(len(data)):
    ...
    if target in li_train:
        ...
        train_llm1_rat.append(str(data.llama_rational.values[i]))
        train_llm2_rat.append(str(data.mistral_rational.values[i]))
        train_stances.append(str(data.true_stance.values[i]))
        train_llm1_stance.append(str(data.llama_stance.values[i]))
...
for i in range(len(data)):
    ...
    if target in li_test:
        ...
        test_llm1_rat.append(str(data.llama_rational.values[i]))
        test_llm2_rat.append(str(data.mistral_rational.values[i]))
        test_stances.append(str(data.true_stance.values[i]))
        test_llm1_stance.append(str(data.llama_stance.values[i]))
...
train_dataset = CreateDataset(train_ids, train_texts, train_llm1_rat, train_llm2_rat, train_aux_features, train_targets, train_comet_intents,train_llm_intents, train_stances, train_llm1_stance, train_llm2_stance,tweet_tokenizer, tokenizer, max_length=128)
val_dataset = CreateDataset(val_ids, val_texts, val_llm1_rat, val_llm2_rat, val_aux_features, val_targets, val_comet_intents,val_llm_intents, val_stances, val_llm1_stance, val_llm2_stance,tweet_tokenizer,tokenizer, max_length=128)
test_dataset = CreateDataset(test_ids, test_texts, test_llm1_rat, test_llm2_rat, test_aux_features, test_targets, test_comet_intents,test_llm_intents, test_stances, test_llm1_stance, test_llm2_stance,tweet_tokenizer,tokenizer, max_length=128)
...
loaded_rational = RationalModel(bertweet_model,bert_model, hidden_dim_rational, rank,num_heads,hidden_size,dropout,output_size)
...
stance_model = StanceModel_Stage2(bertweet_model,rank,num_heads,hidden_size,num_intermediate_outputs,dropout,num_classes)
...
for epoch in range(num_epochs):
    ...
    for ids,text_ids,text_mask,comet_intent_ids,comet_intent_mask,llm_intent_ids,llm_intent_mask,llm1_rational_binary,llm2_rational_binary,aux_feature,true_stance,llm1_stance,llm2_stance in train_loader:
        ...
        predictions = stance_model(text_ids,text_mask,aux_feature,loaded_rational_preds,llm1_rational_binary,llm2_rational_binary,llm1_stance,llm2_stance)
        predictions=predictions.to(torch.float)
        # Convert predicted labels in form of 0,1,2 to logits to calculate loss
        one_hot_labels = torch.eye(3)[predictions.long()]
        stance_logits = torch.log(one_hot_labels + 1e-9)  # Add a small epsilon to avoid taking the logarithm of zero
    ...
    with torch.no_grad():
        for ids,text_ids,text_mask,comet_intent_ids,comet_intent_mask,llm_intent_ids,llm_intent_mask,llm1_rational_binary,llm2_rational_binary,aux_feature,true_stance,llm1_stance,llm2_stance in val_loader:
            ...
            # Convert predicted labels in form of 0,1,2 to logits to calculate loss
            one_hot_labels = torch.eye(3)[predictions.long()]
            stance_logits = torch.log(one_hot_labels + 1e-9)
            # Calculate loss for Stance task
            stance_loss = stance_criterion(stance_logits, true_stance)
...
loaded_stance_stage2 = StanceModel_Stage2(bertweet_model,rank,num_heads,hidden_size,num_intermediate_outputs,dropout,num_classes)
...
with torch.no_grad():
    for ids,text_ids,text_mask,comet_intent_ids,comet_intent_mask,llm_intent_ids,llm_intent_mask,llm1_rational_binary,llm2_rational_binary,aux_feature,true_stance,llm1_stance,llm2_stance in test_loader:
        ...
        predictions = loaded_stance_stage2(text_ids,text_mask,aux_feature,loaded_rational_preds,llm1_rational_binary,llm2_rational_binary,llm1_stance,llm2_stance)
        predictions = predictions.cpu().numpy()
        true_stance=true_stance.cpu().numpy()
...
with open("predictions/Stance_ensemble_conf.csv", "w", newline="", encoding="utf-8") as csvfile:
    fieldnames = ["True", "Prediction"]
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()

    for text, prediction in zip(test_targets, test_predictions):
        writer.writerow({
            "True": text,
